...
def index():
	...
	if (not isempty(city)) and (not isempty(dist)):
		...
		try:
			...
			rs = redis.hscan_iter("idx:city_by_name", match=f"{city}*")
			...
			brewcoords = redis.geosearch("idx:breweries", longitude=lng, latitude=lat, radius=dist, unit="mi", sort="ASC", withcoord=True, count=10)
			...
			for c in brewcoords:
				...
				b = redis.hmget(f"brw:{bid}", "name", "city")
				breweries.append({ "name": b[0], "city": b[1], "lat": blat, "lng": blng })
